jQuery(function ($) {
  let $locations_list = $("#bookly-locations"),
    ...
    $location_color = $("#bookly-location-color", $location_modal), // Ajout de la couleur
    ...
    elements = {
      draghandle: $(
        '<i class="fas fa-fw fa-bars text-muted bookly-cursor-move bookly-js-draghandle" />',
        { title: BooklyL10n.reorder }
      ).get(0).outerHTML,
      edit_btn: $(
        '<button type="button" class="btn btn-default" data-action="edit">'
      )
        .append(
          $('<i class="far fa-fw fa-edit mr-lg-1" />'),
          '<span class="d-none d-lg-inline">' + BooklyL10n.edit + "…</span>"
        )
        .get(0).outerHTML,
    },
    row;
  ...
  $.each(
    BooklyL10n.datatables.locations.settings.columns,
    function (column, show) {
      if (show) {
        if (column === "color") {
          columns.push({
            data: column,
            ...
            render: function (data, type, row, meta) {
              if (data === 0) {
                ...
                return $.fn.dataTable.render
                  .text()
                  .display(BooklyL10n.staff.collection[row.staff_id].full_name);
              } else {
  ...
  var dt = $locations_list
    ...
    .on("row-reordered", function (e, diff, edit) {
      ...
      $.ajax({
        url: ajaxurl,
        type: "POST",
        data: booklySerialize.buildRequestData(
          "bookly_locations_update_locations_position",
          data
        ),
        dataType: "json",
  ...
  $save_button.on("click", function (e) {
    ...
    $.ajax({
      ...
      type: "POST",
      data: booklySerialize.buildRequestData(
        "bookly_locations_save_location",
        data
      ),
      dataType: "json",
      success: function (response) {
