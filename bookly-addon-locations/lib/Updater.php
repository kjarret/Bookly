...
class Updater extends \Bookly\Lib\Base\Updater
{
    ...
    function update_2_1()
    {
        ...
        foreach ($tables as $table) {
            $query .= sprintf(
                '`%s` TO `%s`, ',
                $this->getTableName('ab_' . $table),
                $this->getTableName('bookly_' . $table)
            );
        }
    ...
    function update_1_1()
    {
        ...
        $wpdb->query(
            'ALTER TABLE `' . $this->getTableName('ab_customer_appointments') . '` 
             ADD CONSTRAINT
                FOREIGN KEY (`location_id`)
    ...
    // Nouvelle méthode pour ajouter la colonne "color"
    public function update_5_0()
    {
        $this->alterTables(array(
            'bookly_locations' => array(
                'ALTER TABLE `%s` ADD COLUMN `color` VARCHAR(7) NOT NULL DEFAULT "#cccccc" AFTER `info`',
            ),
        ));
    }
}
