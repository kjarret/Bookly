  ...
  let Calendar = function ($container, options) {
    ...
    let settings = {
        ...
        timeline: {
          ...
          text: {
            html:
              ...
              obj.options.l10n.timeline,
          },
          click: function (e) {
            let headerToolbar = calendar.getOption("headerToolbar"),
              view = calendar.getOption("view");
            headerToolbar.end =
              "dayGridMonth,timeGridWeek,resourceTimeGridDay,grid listWeek";
        ...
        grid: {
          ...
          click: function () {
            let headerToolbar = calendar.getOption("headerToolbar"),
              view = calendar.getOption("view");
            headerToolbar.end =
              "resourceTimelineMonth,resourceTimelineWeek,resourceTimelineDay,timeline listWeek";
            calendar.setOption("headerToolbar", headerToolbar);
      ...
      eventMouseEnter: function (arg) {
        ...
        if (arg.event.display === "auto" && arg.view.type !== "listWeek") {
          ...
          if (offset.left > window.innerWidth / 2) {
            $popover = $(
              '<div class="bookly-popover bs-popover-top bookly-ec-popover bookly-popover-right">'
            );
            $arrow = $(
              '<div class="arrow" style="right: 8px;"></div><div class="bookly-arrow-background"></div>'
            );
          } else {
            $popover = $(
              '<div class="bookly-popover bs-popover-top bookly-ec-popover">'
            );
            $arrow = $(
              '<div class="arrow" style="left:8px;"></div><div class="bookly-arrow-background"></div>'
            );
          }
          let $body = $('<div class="popover-body">');
          let $buttons = existsAppointmentForm ? popoverButtons(arg) : "";
          $body
            .append(arg.event.extendedProps.tooltip)
            .append($buttons)
            .css({ minWidth: "200px" });
          ...
          let popover_height = $popover.outerHeight(),
            $calendar_container = $event.closest(".ec-body").length
              ? $event.closest(".ec-body")
              : $event.closest(".ec-all-day"),
          ...
          if (container_top > offset.top - popover_height) {
            ...
            $popover.css(
              "top",
              Math.max(container_top, offset.top) -
                $(document).scrollTop() +
                "px"
            );
            if ($popover.hasClass("bookly-popover-right")) {
              $popover
                .removeClass("bs-popover-top")
                .addClass("bs-popover-left");
              ...
              $popover.css(
                "left",
                Math.min(
                  offset.left - 7 + event_width,
                  $calendar_container.offset().left +
                    $calendar_container.outerWidth() -
                    $popover.outerWidth() -
                    32
                ) + "px"
              );
            ...
            let top = Math.max(
              popover_height + 40,
              Math.max(container_top, offset.top) - $(document).scrollTop()
            );
            ...
            if ($popover.hasClass("bookly-popover-right")) {
              $popover.css(
                "left",
                offset.left + event_width - $popover.outerWidth() + "px"
              );
            } else {
      ...
      eventContent: function (arg) {
        ...
        let event = arg.event;
        let props = event.extendedProps; // Propriétés envoyées depuis PHP
        let nodes = [];

        // Récupération des informations
        let timeText = props.header_text || arg.timeText;
        let staffName = props.staff_name ? props.staff_name : ""; // Si absent, chaîne vide
        let customerName = props.customer_name ? props.customer_name : ""; // Si absent, chaîne vide

        // Construction de la chaîne d'affichage avec gestion des tirets
        let displayText = timeText;
        if (staffName || customerName) {
          displayText += ` • ${staffName}${
            staffName && customerName ? " - " : ""
          }${customerName}`;
        }

        // Création de l'élément pour l'affichage
        let $time = $('<div class="ec-event-time"/>').append(displayText);
        ...
        // Gestion des styles selon le statut
            ...
            function (event) {
              if (event == "refresh") {
                ...
                if (event.start === null) {
                  ...
                  if (
                    visible_staff_id == event.resourceId ||
                    visible_staff_id == 0
                  ) {
                    ...
                    jQuery(
                      "li > a[data-staff_id=" + event.resourceId + "]"
                    ).click();
                  }
                }
              }
    ...
    function popoverButtons(arg) {
      const $buttons =
        arg.view.type === "listWeek"
          ? $('<div class="mt-2 d-flex"></div>')
          : $(
              '<div class="mt-2 d-flex justify-content-end border-top pt-2"></div>'
            );
      let props = arg.event.extendedProps;
      $buttons.append(
        $('<button class="btn btn-success btn-sm mr-1">').append(
          '<i class="far fa-fw fa-edit">'
        )
      );
      if (
        obj.options.l10n.recurring_appointments.active == "1" &&
        props.series_id
      ) {
        $buttons.append(
          $('<a class="btn btn-default btn-sm mr-1">')
            .append('<i class="fas fa-fw fa-link">')
      ...
      if (obj.options.l10n.waiting_list.active == "1" && props.waitlisted > 0) {
        $buttons.append(
          $('<a class="btn btn-default btn-sm mr-1">')
            .append('<i class="far fa-fw fa-list-alt">')
      ...
      if (obj.options.l10n.packages.active == "1" && props.package_id > 0) {
        $buttons.append(
          $('<a class="btn btn-default btn-sm mr-1">')
            .append('<i class="far fa-fw fa-calendar-alt">')
      ...
      $buttons.append(
        $('<a class="btn btn-danger btn-sm text-white">')
          .append('<i class="far fa-fw fa-trash-alt">')
          ...
          .on("click", function (e) {
            ...
            if (
              ...
              $(document.body).trigger("recurring_appointments.delete_dialog", [
                calendar,
                arg.event,
              ]);
            } else {
