        ...
        <div class="card-body">
            <?php if ($staff_members): ?>
                <div class="form-row justify-content-xl-end justify-content-center">
                    ...
                    <?php Proxy\Locations::renderCalendarLocationFilter() ?> <!-- Ajout du filtre de localisation -->
                    <div class="col-sm-auto mb-2">
                        <ul id="bookly-js-services-filter" data-icon-class="far fa-dot-circle" data-align="right"
                    ...
                    <?php if (Common::isCurrentUserSupervisor()): ?>
                        <div class="col-sm-auto mb-2">
                            <ul id="bookly-js-staff-filter" data-align="right"
                    ...
                    <div class="col-sm-auto mb-2 text-center">
                        <div class="btn-group">
                            <button type="button"
                                class="btn <?php echo esc_attr($refresh_rate > 0 ? 'btn-success' : 'btn-default') ?>"
                                id="bookly-calendar-refresh"><i class="fas fa-sync-alt"></i></button>
                            <button type="button"
                                class="btn <?php echo esc_attr($refresh_rate > 0 ? 'btn-success' : 'btn-default') ?> bookly-dropdown-toggle bookly-dropdown-toggle-split"
                                data-toggle="bookly-dropdown" aria-haspopup="true" aria-expanded="false"></button>
                            <div class="bookly-dropdown-menu pb-0 bookly-dropdown-menu-right overflow-hidden">
                                <h6 class="bookly-dropdown-header"><?php esc_html_e('Auto-refresh Calendar', 'bookly') ?>
                                </h6>
                                <div class="bookly-dropdown-divider"></div>
                                <?php Components\Controls\Inputs::renderRadioGroup(
                                    null,
                                    null,
                                    array(
                ...
                <div
                    class="nav-scrollable nav-scrollable-pills justify-content-center justify-content-xl-start bookly-js-staff-pills <?php if (count($staff_members) == 1): ?>d-none<?php endif ?>">
                    <ul class="col-auto nav nav-pills flex-nowrap">
                        ...
                        <?php foreach ($staff_members as $staff): ?>
                            <li class="nav-item mr-2 text-nowrap<?php if (!Common::isCurrentUserSupervisor()): ?> d-none<?php endif ?>"
                                style="display:none;">
                                <a class="nav-link d-block p-2 text-center" href="#"
                                    data-staff_id="<?php echo esc_attr($staff->getId()) ?>">
                                    <?php if ($image = $staff->getImageUrl('thumbnail')): ?>
                                        <span class="rounded-circle d-flex overflow-hidden m-auto"
                                            style="height: 40px; width: 40px;">
                                            <img src="<?php echo esc_attr($image) ?>"
                                                alt="<?php echo esc_attr($staff->getFullName()) ?>" class="d-block mx-auto"
                                                style="max-width: 40px; max-height: 40px; align-self: center;" />
                                        </span>
                                    <?php else: ?>
                                        <i class="far fa-user-circle fa-2x d-block mx-auto font-weight-bold"
            ...
            <div class="mt-3 position-relative">
                <?php if ($staff_members): ?>
                    <input type="hidden" value="<?php echo esc_attr($staff_members[0]->getId()) ?>"
                        id="bookly-js-staff-id" data-name="<?php echo esc_attr($staff_members[0]->getFullName()) ?>" />
                ...
                <?php else: ?>
                    <!-- Message d'accueil si aucun membre ou service -->
                <?php endif ?>
            </div>
        </div>
    </div>
