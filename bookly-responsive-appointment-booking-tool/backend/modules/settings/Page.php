...
class Page extends Lib\Base\Ajax
{
    ...
    public static function render()
    {
        ...
        if (!empty($_POST) && self::csrfTokenValid()) {
            ...
            switch (self::parameter('tab')) {
                case 'calendar':  // Calendar form.
                    $coloring_mode = self::parameter('bookly_cal_coloring_mode');
                    if (in_array($coloring_mode, ['service', 'status', 'location'])) {
                        update_option('bookly_cal_coloring_mode', $coloring_mode);
                    }
                    ...
                    foreach (self::parameter('status') as $status => $color) {
                        if (in_array($status, [CustomerAppointment::STATUS_PENDING, CustomerAppointment::STATUS_APPROVED, CustomerAppointment::STATUS_CANCELLED, CustomerAppointment::STATUS_REJECTED, 'mixed'])) {
        ...
        wp_localize_script('bookly-settings.js', 'BooklyL10n', array(
            ...
            'colors_location' => $values['colors_location'],
        ...
        foreach (Lib\Proxy\CustomStatuses::prepareAllStatuses(array(Lib\Entities\CustomerAppointment::STATUS_APPROVED, Lib\Entities\CustomerAppointment::STATUS_PENDING, )) as $status) {
        ...
        $values['colors_location'] = Lib\Entities\Location::query()
